...
- **Cấu trúc thư mục độc lập:**
  - `frontend/`: Toàn bộ source code React/Vite. Khởi chạy ở port 5173/5174. Tương tác với Backend thông qua biến môi trường `import.meta.env.VITE_API_URL` (mặc định fallback về `http://localhost:3000`). Đã được **Deploy thành công lên Vercel** (với cấu hình Root Directory là `frontend`).
  - `backend/`: Toàn bộ source code NestJS. Khởi chạy ở port 3000. Cung cấp RESTful API và nhận Webhook. Yêu cầu kết nối CSDL Supabase PostgreSQL qua 2 biến `DATABASE_URL` và `DIRECT_URL`.
...
3. **Cải thiện hệ thống & Deploy Backend:**
   - Hoàn thành việc deploy Backend NestJS lên các dịch vụ như Render hoặc Railway.
   - Bổ sung cấu hình CORS trên Backend cho phép domain Vercel hiện tại của Frontend gọi API thành công.
   - Tối ưu luồng upload ảnh: Nếu deploy Backend lên Cloud, cần cấu hình đẩy file lên S3/Cloudinary thay vì lưu vào thư mục `frontend/public/images` (do server cloud thường không lưu file tĩnh vĩnh viễn).
